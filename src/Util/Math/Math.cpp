...
inline void CUtil_Math::CreateVector(const Vector& angle, Vector& vector)
{
	float pitch, yaw, tmp;
...
float CUtil_Math::GetFov(const Vector& angle, const Vector& source, const Vector& destination)
{
	Vector ang, aim;
	float mag, u_dot_v;
	ang = GetAngleToPosition(source, destination);
...
Vector CUtil_Math::CalcAngle(const Vector& source, const Vector& destination, bool clamp = true)
{
	Vector angles = {};
