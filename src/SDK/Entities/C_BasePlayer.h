...
public:
	...
	Vector& m_vecLadderNormal() {
		return *reinterpret_cast<Vector*>(reinterpret_cast<DWORD>(this) + 0x112C);
	}

	void SetCurrentCommand(CUserCmd* pCmd) {
		static int nOffset = H::NetVar.Get("CBasePlayer", "m_hConstraintEntity") - 4;
		*reinterpret_cast<CUserCmd**>(reinterpret_cast<DWORD>(this) + nOffset) = pCmd;
	}

	int& m_nButtons() {
		static int nOffset = H::NetVar.Get("CBasePlayer", "m_hConstraintEntity") - 8;
	...
	*/
	...
	int m_afButtonLast() {
		static int nOffset = H::NetVar.Get("CBasePlayer", "m_hConstraintEntity") - 20;
		return *reinterpret_cast<int*>(reinterpret_cast<DWORD>(this) + nOffset);
	}
};
